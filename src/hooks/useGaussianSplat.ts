...
import { getProjectionMatrix, multiply4, invert4, rotate4, translate4 } from '@/lib/math';
...
export function useGaussianSplat(source: string | File | null): UseGaussianSplatReturn {
  ...
  useEffect(() => { totalSplatsRef.current = totalSplats; }, [totalSplats]);
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      if (!ru) return;
      // High-res framebuffer for sharp rendering on Retina displays
      const dpr = Math.min(window.devicePixelRatio || 1, 2);
      canvas.width = Math.round(window.innerWidth * dpr);
      canvas.height = Math.round(window.innerHeight * dpr);
      gl.viewport(0, 0, canvas.width, canvas.height);
      // Shader uses CSS-pixel units so projection stays geometrically correct
    ...
    const handleBlur = () => { activeKeysRef.current = []; };
    ...
    const frame = (now: number) => {
      ...
      if (animationPhaseRef.current === 'revealing') {
        ...
        if (elapsedMs >= TOTAL_DURATION) {
          ...
          setPhase('ready');
          // Force a final depth sort so the model is correctly ordered once the wave ends
          forceSortRef.current = true;
          lastViewProjRef.current = [];
        }
        showEverything = 0;
      ...
      if (inv2Result && projectionMatrixRef.current.length > 0) {
        ...
        if (actualViewMatrix) {
          ...
          if (isRevealing) {
            // During wave reveal animation: ONLY sort once at the very start (force).
            // Continuous sorting during the wave breaks visual coherence because
            // splats appear/disappear by distance, not depth order.
            const shouldSort = forceSortRef.current;
            if (shouldSort) {
              worker.postMessage({ view: viewProj, force: true });
              lastViewProjRef.current = viewProj;
              forceSortRef.current = false;
            }
          } else {
            // downloading or ready: sort whenever camera moves
            const shouldSort = forceSortRef.current || viewChanged;
            if (shouldSort) {
              const force = forceSortRef.current;
              worker.postMessage({ view: viewProj, force });
              lastViewProjRef.current = viewProj;
              if (forceSortRef.current) forceSortRef.current = false;
            }
          }
